...
def lizardTree(T=nx.Graph(),ids={},scale_factor=SCALE_FACTOR):
    set_id=count()
    ...
    return pts,ids,T, tree_distances(T)

def threeNodesTree(T=nx.Graph(),ids={}, scale_factor=SCALE_FACTOR):
    set_id=count()
    #TODO: do the actual NLopt for circle packing
...
def antennaBeetleTree(T=nx.Graph(),ids={},scale_factor=SCALE_FACTOR):
    #TODO: do the actual NLopt for circle packing
    set_id=count()
...
def beetleTree(T=nx.Graph(),ids={},scale_factor=SCALE_FACTOR):
    set_id=count()
    #TODO: do the actual NLopt for circle packing
